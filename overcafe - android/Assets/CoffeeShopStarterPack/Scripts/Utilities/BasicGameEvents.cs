...
namespace PW {
    public class BasicGameEvents : MonoBehaviour
    {
        ...
        public delegate void OnOrderCompleted(int ID,float percentageSuccess, bool isSpecial);

        public static event OnOrderCompleted onOrderCompleted;

        public static void RaiseOnOrderCompleted(int ID,float percentageSuccess, bool isSpecial)
        {
            if (onOrderCompleted != null)
            {
                onOrderCompleted.Invoke(ID,percentageSuccess,isSpecial);
            }
        }
        #endregion
